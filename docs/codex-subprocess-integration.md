...
> 版本：1.0 | Codex CLI v2.1.x | 更新：2026-05-10

本文档描述如何通过子进程方式对接 Codex CLI，实现**程序化调用**和**中途权限控制**。
适用于开发机器人、自动化工具、IDE 插件等需要与 Codex 交互的场景。
...
**核心思路**：将 Codex CLI 作为子进程启动，通过 stdin/stdout 的 JSON 行协议进行双向通信。CLI 处理 AI 模型调用，你的程序负责发送用户消息和处理权限请求。
...
```bash
# 模型选择
--model gpt-5.2             # 或使用 Codex CLI 支持的其他模型
...
当 Codex 需要执行需要权限的工具时，CLI 通过 stdout 发送：
...
如果从 Codex 内部启动子进程（如本项目中），**必须删除** `CODEXCODE` 环境变量，否则 CLI 会检测到嵌套会话并异常。

```typescript
const env = { ...process.env };
delete env.CODEXCODE;  // 必须！
```
...
const proc = spawn("codex", [
  ...
  cwd: process.cwd(),
  env: { ...process.env, CODEXCODE: undefined },
  stdio: ["pipe", "pipe", "pipe"],
});
...
proc = subprocess.Popen(
    ...
    env={**os.environ, "CODEXCODE": ""},  # 清除嵌套检测
    cwd=os.getcwd(),
)
...
| 项目 | 语言 | 说明 |
|------|------|------|
| [cc-connect](https://github.com/chenhg5/cc-connect) | Go | 完整的 Codex 代理/网关，权限控制参考实现 |
| Codex CLI JSON 输出 | Go | 本项目直接解析 `codex exec --json` 事件流 |
