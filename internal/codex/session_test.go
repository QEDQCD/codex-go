...
func TestFilterOutEnv(t *testing.T) {
	env := []string{"PATH=/usr/bin", "CODEXCODE=something", "HOME=/home"}
	result := filterOut(env, "CODEXCODE")
	if len(result) != 2 {
		t.Errorf("expected 2 entries, got %d: %v", len(result), result)
	}
	for _, e := range result {
		if len(e) >= 10 && e[:10] == "CODEXCODE" {
			t.Errorf("CODEXCODE should have been filtered: %s", e)
		}
	}
}
...
func TestStartSession_ValidCLI(t *testing.T) {
	...
	if sess.PID() == 0 {
		t.Errorf("expected app-server child process after start")
	}
	// Clean up
	sess.Stop()
}
...
func TestSendMessage(t *testing.T) {
	if os.Getenv("CODEX_GO_INTEGRATION") != "1" {
		t.Skip("set CODEX_GO_INTEGRATION=1 to run liveCodex CLI test")
	}
	path, err := FindCodexCLI()
	if err != nil {
		t.Skip("codex not found")
...
func TestRespondPermission_Allow(t *testing.T) {
	...
	if err := sess.RespondPermission("test-req-id", true, ""); err != nil {
		t.Errorf("expected permission response write to be supported, got %v", err)
	}
}
...
func TestRespondPermission_Deny(t *testing.T) {
	...
	if err := sess.RespondPermission("test-req-id", false, "Rejected by user"); err != nil {
		t.Errorf("expected permission response write to be supported, got %v", err)
	}
}
...
func TestPermissionModeMapping(t *testing.T) {
	cases := []struct {
		...
		{mode: "default", approval: "on-request", sandbox: "workspace-write"},
		{mode: "on-request", approval: "on-request", sandbox: "workspace-write"},
		{mode: "never", approval: "never", sandbox: "workspace-write"},
		{mode: "plan", approval: "never", sandbox: "read-only"},
		{mode: "bypassPermissions", approval: "never", sandbox: "danger-full-access"},
	}
	for _, tc := range cases {
		if got := approvalPolicyForPermMode(tc.mode); got != tc.approval {
			t.Fatalf("%s approval: expected %s got %s", tc.mode, tc.approval, got)
		}
		if got := sandboxModeForPermMode(tc.mode); got != tc.sandbox {
			t.Fatalf("%s sandbox: expected %s got %s", tc.mode, tc.sandbox, got)
		}
	}
}
