...
func CodexCodeSkillsDir() string {
...
端口配置在 ` + "`~/.codex-go/config.json`" + ` 的 ` + "`web_port`" + ` 字段中，发送请求前先读取：

` + "```" + `bash
PORT=$(grep -o '"web_port":[[:space:]]*[0-9]*' ~/.codex-go/config.json | grep -o '[0-9]*')
` + "```" + `
...
func DiscoverCodexCodeSkills() []SkillConfig {
	dir := CodexCodeSkillsDir()
	if dir == "" {
		return nil
	}
	return discoverSkillsFrom(dir)
...
func (c *Config) InjectSkills(workDir string) {
	...
	projectSkillsDir := filepath.Join(workDir, ".codex", "skills")
