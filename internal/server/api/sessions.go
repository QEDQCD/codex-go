...
func registerCodexRoutes(r *gin.RouterGroup, st *store.Store, br *bridge.Bridge) {
	r.GET("/codex/path", func(c *gin.Context) {
		path, err := codex.FindCodexCLI()
		c.JSON(http.StatusOK, gin.H{"path": path, "error": errToStr(err)})
	})

	r.POST("/codex/path", func(c *gin.Context) {
		var req struct {
		...
		version, err := codex.ValidateCodexCLI(req.Path)
		if err != nil {
			c.JSON(http.StatusBadRequest, gin.H{"error": err.Error()})
			return
		}
	...
	r.POST("/codex/auto-detect", func(c *gin.Context) {
		path, err := codex.FindCodexCLI()
		if err != nil {
			c.JSON(http.StatusNotFound, gin.H{"error": "codex CLI not found"})
			return
		}
		version, err := codex.ValidateCodexCLI(path)
...
func registerSessionRoutes(r *gin.RouterGroup, st *store.Store, br *bridge.Bridge) {
	...
	r.GET("/stats", func(c *gin.Context) {
		...
		historySessions, _ := codex.DiscoverSessions()
	...
	r.GET("/sessions", func(c *gin.Context) {
		storeSessions := br.RunningSessions()
		activeID := br.ActiveSessionID()
		var result []gin.H
	...
	r.GET("/sessions/active/log", func(c *gin.Context) {
		activeID := br.ActiveSessionID()
		if activeID == "" {
			c.JSON(http.StatusOK, []codex.LogEntry{})
			return
		}
		entries, err := codex.ReadLog(activeID)
		if err != nil {
	...
	r.POST("/sessions/start", func(c *gin.Context) {
		...
		if err := br.StartSession(codex.StartOptions{WorkDir: req.WorkDir, Model: req.Model, Name: req.Name}); err != nil {
			c.JSON(http.StatusInternalServerError, gin.H{"error": err.Error()})
			return
		}
	...
	r.POST("/sessions/:id/resume", func(c *gin.Context) {
		...
		if err := br.StartSession(codex.StartOptions{WorkDir: hs.ProjectPath, Name: hs.FirstPrompt, ResumeID: id}); err != nil {
			c.JSON(http.StatusInternalServerError, gin.H{"error": err.Error()})
			return
		}
	...
	r.DELETE("/sessions/:id", func(c *gin.Context) {
		...
		if hs := codex.FindSession(id); hs != nil {
