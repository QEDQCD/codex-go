...
func registerPushRoutes(r *gin.RouterGroup, cfg *config.Config) {
	r.GET("/push/types", func(c *gin.Context) {
		c.JSON(http.StatusOK, gin.H{"types": []gin.H{
			...
			{"key": "codex_response", "label": "Codex 响应内容", "required": false},
...
func registerSettingsRoutes(r *gin.RouterGroup, cfg *config.Config) {
	...
	r.PUT("/settings", func(c *gin.Context) {
		...
		if req.CodexCLIPath != "" {
			cfg.CodexCLIPath = req.CodexCLIPath
		}
		if req.AutoFindCodex != nil {
			cfg.AutoFindCodex = *req.AutoFindCodex
		}
		...
		cfg.CodexEnvVars = req.CodexEnvVars
		cfg.Save()
		c.JSON(http.StatusOK, cfg)
	})
	...
	r.PUT("/bot-commands", func(c *gin.Context) {
		var req struct {
			Commands []config.BotCommand `json:"commands"`
		}
		if err := c.ShouldBindJSON(&req); err != nil {
			c.JSON(http.StatusBadRequest, gin.H{"error": err.Error()})
			return
	...
	r.PUT("/skills", func(c *gin.Context) {
		var req struct {
			Skills []config.SkillConfig `json:"skills"`
		}
		if err := c.ShouldBindJSON(&req); err != nil {
			c.JSON(http.StatusBadRequest, gin.H{"error": err.Error()})
			return
	...
	r.GET("/skills/available", func(c *gin.Context) {
		ccSkills := config.DiscoverCodexCodeSkills()
