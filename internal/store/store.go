...
func Open() (*Store, error) {
	...
	dbDir := filepath.Join(home, ".codex-go")
	if err := os.MkdirAll(dbDir, 0755); err != nil {
		return nil, err
	}
	dbPath := filepath.Join(dbDir, "codex-go.db")
...
const tableDef = `CREATE TABLE IF NOT EXISTS sessions (
	...
	codex_pid INTEGER NOT NULL DEFAULT 0,
...
const allCols = `seq, id, name, work_dir, model, status, codex_pid, created_at, last_active_at, history_path, message_count, git_branch`
...
func (s *Store) migrate() error {
	...
	if !strings.Contains(schema, "codex_pid") {
		if strings.Contains(schema, "claude_pid") {
			if _, err := s.db.Exec(`ALTER TABLE sessions RENAME COLUMN claude_pid TO codex_pid`); err != nil {
				return err
			}
			schema = strings.ReplaceAll(schema, "claude_pid", "codex_pid")
		} else {
			if _, err := s.db.Exec(`ALTER TABLE sessions ADD COLUMN codex_pid INTEGER NOT NULL DEFAULT 0`); err != nil {
				return err
			}
			schema += " codex_pid"
		}
	...
	if strings.Contains(schema, "id TEXT PRIMARY KEY") {
		statements := []string{
			tableDef,
			`INSERT INTO sessions (seq, id, name, work_dir, model, status, codex_pid, created_at, last_active_at, history_path)
			 SELECT rowid, id, name, work_dir, model, status, codex_pid, created_at, last_active_at, history_path FROM sessions_old WHERE id IS NOT NULL AND id != ''`,
		}
...
func (s *Store) InsertSession(sess *Session) error {
	...
	_, err := s.db.Exec(
		`INSERT INTO sessions (id, name, work_dir, model, status, codex_pid, created_at, last_active_at, history_path, message_count, git_branch)
		 VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
		 ON CONFLICT(id) DO UPDATE SET
			...
			codex_pid = ?,
			last_active_at = ?`,
		sess.ID, sess.Name, sess.WorkDir, sess.Model, sess.Status, sess.CodexPID,
...
func (s *Store) UpdateSessionStatus(id, status string, pid int) error {
	...
	defer s.mu.Unlock()
	_, err := s.db.Exec(
		`UPDATE sessions SET status = ?, codex_pid = ?, last_active_at = CURRENT_TIMESTAMP WHERE id = ?`,
		status, pid, id,
	)
	return err
}
...
func scanSession(row *sql.Row) (*Session, error) {
	var sess Session
	err := row.Scan(&sess.Seq, &sess.ID, &sess.Name, &sess.WorkDir, &sess.Model, &sess.Status, &sess.CodexPID, &sess.CreatedAt, &sess.LastActiveAt, &sess.HistoryPath, &sess.MessageCount, &sess.GitBranch)
	if err != nil {
		return nil, err
	}
	return &sess, nil
...
func (s *Store) ListSessions() ([]Session, error) {
	...
	for rows.Next() {
		var sess Session
		if err := rows.Scan(&sess.Seq, &sess.ID, &sess.Name, &sess.WorkDir, &sess.Model, &sess.Status, &sess.CodexPID, &sess.CreatedAt, &sess.LastActiveAt, &sess.HistoryPath, &sess.MessageCount, &sess.GitBranch); err != nil {
			return nil, err
		}
