...
const tabs = [
  ...
  { id: 'cli', icon: 'terminal', label: 'Codex CLI' },
...
export default function Settings() {
  ...
  useEffect(() => {
    ...
    api.getCodexPath().then(data => setCodexInfo(data)).catch(() => {})
  ...
  const handleAutoDetect = async () => {
    setCliLoading(true)
    try {
      const data = await api.autoDetectCodex()
      setCodexInfo(data)
    } catch {
      // ignore
    } finally {
  ...
  const handleValidate = async () => {
    if (!codexInfo?.path) return
    setCliLoading(true)
    try {
      const data = await api.setCodexPath(codexInfo.path)
      setCodexInfo(prev => prev ? { ...prev, valid: data.valid, version: data.version } : data)
    } catch {
      // ignore
    } finally {
      setCliLoading(false)
  ...
  return (
    <div className="p-8 max-w-[1440px] mx-auto w-full">
      <div className="mb-8">
        ...
        <p className="text-[14px] text-on-surface-variant">管理 codex-go 的全局配置和偏好设置。</p>
      </div>
        ...
        <div className="col-span-12 md:col-span-9">
            ...
            <div className="bg-surface-container border border-outline-variant rounded-lg p-6">
              ...
              <div className="space-y-6">
                ...
                <div className="space-y-4">
                  ...
                  <div className="flex items-center justify-between p-3 bg-surface-container-low rounded-lg">
                    <div className="min-w-0 flex-1">
                      <p className="text-[14px] text-on-surface">自动查找 Codex CLI</p>
                      <p className="text-[12px] text-on-surface-variant">启动时自动在系统 PATH 中查找 Codex CLI</p>
                    </div>
                    <div className="shrink-0"><Toggle
                      checked={settings.auto_find_codex || false}
                      onChange={v => setSettings({ ...settings, auto_find_codex: v })}
                    /></div>
                  </div>
                  <div className="p-3 bg-surface-container-low rounded-lg">
                    ...
                    <p className="text-[11px] text-on-surface-variant mt-2">权限模式控制 Codex app-server 会话中权限请求的处理方式。</p>
                  </div>
                </div>
              </div>
            </div>
          ...
          {activeTab === 'cli' && (
            <div className="bg-surface-container border border-outline-variant rounded-lg p-6">
              <div className="flex items-center justify-between mb-6">
                <h3 className="text-[20px] font-semibold text-primary">Codex CLI 配置</h3>
                {codexInfo?.valid && (
              ...
              <div className="space-y-4">
                ...
                {codexInfo?.version && (
                  <div className="p-3 bg-surface-container-low rounded-lg">
                    ...
                    <p className="font-mono text-[13px] text-on-surface">{codexInfo.version}</p>
                  </div>
                )}
                <button
                  onClick={handleValidate}
                  disabled={cliLoading || !codexInfo?.path}
                ...
                <div className="mt-6 pt-6 border-t border-outline-variant">
                  ...
                  <p className="text-[12px] text-on-surface-variant mb-3">
                    每行一个 KEY=VALUE，以 # 开头的行为注释。这些变量将在启动 Codex 会话时注入到子进程环境。
                  </p>
                  <textarea
                    value={settings.codex_env_vars || ''}
                    onChange={e => setSettings({ ...settings, codex_env_vars: e.target.value })}
                    placeholder={'# 例如:\n# OPENAI_API_KEY=sk-...\n# OPENAI_BASE_URL=https://api.openai.com/v1\n# CODEX_HOME=/root/.codex\n# CODEX_MAX_OUTPUT_TOKENS=16384'}
                    rows={10}
                    spellCheck={false}
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div className="min-w-0 flex-1">
                    ...
                    <input
                      ...
                      min={4}
                      max={15}
                      value={settings.wechat?.send_budget_limit ?? 15}
                      onChange={e => setSettings({
                        ...settings,
                        wechat: { ...(settings.wechat || {} as any), send_budget_limit: parseInt(e.target.value) || 15 }
                      })}
                      className="w-full bg-surface-container-lowest border border-outline-variant rounded px-3 py-2 font-mono text-[13px] text-on-surface focus:outline-none focus:border-primary"
                    />
                    <p className="text-[11px] text-on-surface-variant mt-1">用户每次发消息后，机器人可回复的最大消息条数 (4-15)</p>
                  </div>
                  <div className="min-w-0 flex-1">
          ...
          {activeTab === 'skills' && (
            <div className="bg-surface-container border border-outline-variant rounded-lg p-6">
              ...
              <p className="text-[13px] text-on-surface-variant mb-6">
                管理 Codex 会话中可用的 Skills。启用后，Skill 会在接管会话时自动注入到项目的 <code className="bg-surface-container-lowest px-1 rounded font-mono">[项目]/.codex/skills/</code> 目录。
              </p>
              <div className="space-y-3">
                {skills.map(skill => (
                ...
                {skills.length === 0 && (
                  <p className="text-[13px] text-on-surface-variant text-center py-6">暂无 Skills，点击下方导入或手动添加到 ~/.codex-go/skills/</p>
                )}
              </div>
              ...
              {importDialogOpen && (
                <div className="mt-6 p-4 bg-surface-container-low rounded-lg border border-outline-variant">
                  <h4 className="text-[16px] font-semibold text-primary mb-3">从 Codex 导入 Skills</h4>
                  <p className="text-[12px] text-on-surface-variant mb-4">
                    以下是从 <code className="bg-surface-container-lowest px-1 rounded font-mono">~/.codex/skills/</code> 发现的 Skills，选择要导入的项目：
                  </p>
