    ...
    <div className="p-8 max-w-[1440px] mx-auto w-full">
      ...
      <div className="mb-8">
        ...
        <p className="text-[14px] text-on-surface-variant">
          通过微信扫码绑定您的账户，实现移动端远程管理 Codex 会话。
        </p>
      </div>
            ...
            <div className="p-6">
              ...
              {connected ? (
                <>
                  ...
                  {disconnectConfirm && (
                    <div className="mt-3 p-3 bg-error/5 rounded-lg border border-error/30">
                      <p className="text-[14px] text-on-surface mb-2">确定要断开微信连接吗？</p>
                      <div className="flex justify-end gap-2">
                        <button
                          onClick={() => setDisconnectConfirm(false)}
                          className="px-3 py-1.5 rounded border border-outline-variant text-on-surface-variant hover:text-primary transition-colors text-[13px]"
                        >
                          取消
                        </button>
                        <button
                          onClick={handleDisconnect}
                          className="px-3 py-1.5 rounded bg-error/10 border border-error/30 text-error hover:bg-error/20 transition-colors text-[13px] font-medium"
                        >
                          断开连接
                        </button>
                      </div>
          ...
          {connected && (
            <div className="bg-surface-container border border-outline-variant rounded-lg overflow-hidden">
              ...
              <div className="p-6">
                ...
                <button
                  onClick={() => setBotApiExpanded(!botApiExpanded)}
                  className="flex items-center gap-2 text-[13px] text-tertiary hover:text-tertiary/80 transition-colors"
                >
                  <span className={`material-symbols-outlined text-[16px] transition-transform ${botApiExpanded ? 'rotate-180' : ''}`}>expand_more</span>
                  {botApiExpanded ? '收起详情' : '查看接口详情'}
                </button>
